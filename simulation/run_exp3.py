#!/usr/bin/env python3
"""run_exp3 - EXP-3: overhead, paired same-day sub-study (the pre-registered protocol rev. 4).
...
Prereq: EXP-1c rows must exist in output/exp1_*.csv. In --dry mode a
...
from lib.seeds import seed_from  # noqa: E402
...
def main() -> None:
    ...
    if not source:
        print("No EXP-1c rows found in output/. Run EXP-1 cell 1c first "
              "(or use --dry).")
        return
