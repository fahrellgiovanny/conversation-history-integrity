#!/usr/bin/env python3
"""run_exp7 - EXP-7: external anchors (SIMULATION_PAPER3 rev. 4).
...
release via benchmarks/memsecbench_cases.json.
...
releases; citation and license compliance required):
  ...
  memsecbench: [{"id", "mode": 1..7, "stage": "write|execute|forget",
...
MEMSECBENCH_MANIFEST = config.BENCH_DIR / "memsecbench_cases.json"
...
def run_scenario_d(model, task: dict, dry: bool) -> list:
    ...
    for i, t in enumerate(sc["turns"], start=1):
        ...
        attack = t.get("attack")
        # BUG FIX (2026-08-08): the attack content was never delivered to
        ...
        if attack and mech is not None:
            ...
            # BUG FIX (2026-08-08): the flag must have a behavioral
    ...
    for i, t in enumerate(sc["turns"], start=1):
        ...
        if attack:
            ...
            if kind == "mutation" and mech is not None and i > 1:
                ...
                # BUG FIX (2026-08-08): on flag, the forged content is not
...
def build_tasks_b() -> list:
    ...
    per = config.EXP7["memsecbench_per_mode"]
    tasks = []
    for mode in range(1, config.EXP7["memsecbench_modes"] + 1):
        for s in [x for x in sc if x.get("mode") == mode][:per]:
            for cond in config.EXP7["memsecbench_conditions"]:
                tasks.append({"scenario": s, "cond": cond,
                              "model": config.MODEL_GEMINI,
    ...
    for i, t in enumerate(sc["turns"], start=1):
        ...
        if attack and mech is not None:
            ...
            if kind == "mutation" and i > 1:
                ...
                # BUG FIX (2026-08-08): on flag, do not deliver the forged
        ...
        if mech is not None:
            mech.observe("\n".join(f"User: {k}\nAssistant: {v}"
                                   for k, v in real.items()))
        rows.append(make_row(
            session_id=task["session_id"], exp="EXP-7", cell="memsecbench",
            model=task["model"], protocol="", case_id=sc["id"],
...
def _run_batch(tag: str, tasks: list, fn, args) -> None:
    ...
    reporter.stop = True
    print(f"DONE: {writer.finish()} rows written to output/integrity/{tag}_*.csv")
