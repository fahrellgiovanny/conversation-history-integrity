...
```
python3 run_exp1.py     # L1 scale evaluation (1,212 sessions)
python3 run_exp2.py     # L2/L3 tiers (300 sessions)
...
python3 run_exp6.py     # composition boundary + fragment-size sweep (120)
python3 run_exp7.py     # external anchors: AgentDojo, lifecycle, PoisonedRAG (187)
python3 run_exp8.py     # instruction-override gate family (300)
python3 run_exp8_probe.py   # shape boundary (60, exploratory)
python3 run_t3v2.py     # guard-model variant (60)
```
...
python3 rebuild_judged_union.py
...
- Verdict-mode sensitivity (strict / lenient / "I don't know"):
  `python3 mode_judge.py`
- Cross-family agreement (GPT judge on a 300-row sample):
  `python3 cross_judge.py`
- Flash-vs-pro agreement on a sample:
  `python3 compare_judge_models.py`
- Human-annotation workbook (440 stratified turns):
  `python3 build_annotation_file.py`

## 6. Analysis

```
python3 audit_results.py    # all headline proportions + Wilson 95% CIs
python3 tables.py    # ASV/MR tables for the comparative ranking
```
