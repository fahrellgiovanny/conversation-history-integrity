"""lib.api - model call wrappers with retry and token accounting.
...
class GeminiModel:
    def __init__(self, model: str = MODEL_GEMINI):
        ...
        # worker forever. SDK AUTO-RETRIES ARE OFF (retry_options.attempts=0):
        # silent SDK retries can re-send ambiguous requests. Every retry is
        # controlled by call_with_retry instead.
...
def _call_once_hard(model, prompt: str) -> dict:
    """One call + ONE retry only on clean rejections.

    Retry policy:
    - TIMEOUT (the server may have processed the request): no in-session
      retry - return an error row immediately; the session is purged and
      re-run once at batch end. Retrying here risks processing the request
      twice.
    - Clean rejections (429/503/5xx/connection errors - rejected before
      processing): one retry after 5 s with a fresh client.
    ...
    try:
        ...
        print(f"  TIMEOUT (server may have processed the request): {e} - no in-session retry; "
        ...
        if "Timeout" in type(e).__name__:
            print(f"  TIMEOUT (server may have processed the request): {type(e).__name__} - no retry")
            return {"rawOutput": "", "finishReason": "error",
                    "usageMetadata": {"promptTokens": 0, "completionTokens": 0,
                                      "totalTokens": 0}, "latency_ms": 0.0}
        print(f"  API ERROR (clean rejection): {e}; "
        ...
        try:
            ...
            print("  TIMEOUT on retry - no further retry")
            return {"rawOutput": "", "finishReason": "error",
                    "usageMetadata": {"promptTokens": 0, "completionTokens": 0,
                                      "totalTokens": 0}, "latency_ms": 0.0}
