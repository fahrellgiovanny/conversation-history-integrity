"""lib.resume - batch checkpoint/resume machinery.

Each runner writes unified-schema batches (output/{tag}_batch_NNN.csv).
...
class _NullWriter:
    """Dry-run sink: accepts the same interface as BatchWriter but writes
    nothing. A dry run must never create batch files; if it did, a later
    real run would treat the dry-run sessions as already completed."""
...
def make_writer(tag: str, dry: bool):
    """Return the batch writer for a run; the dry-run sink when dry=True."""
    return _NullWriter() if dry else BatchWriter(tag)
